...
export const ListOfSeller: React.FC = () => {
  ...
  const [modifyingSeller, setModifyingSeller] = useState<BuyerLead | null>(null);
  ...
  return (
    <div className="space-y-6 pb-12">
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-100/80 border-b border-slate-200 text-slate-700 font-semibold uppercase text-[11px] tracking-wider">
                <th className="px-4 py-3 text-indigo-900">Action</th>
                ...
                filteredSellers.map((seller) => (
                  <tr key={seller.id} className="hover:bg-slate-50/70 transition">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <button
                        id={`btn-view-edit-modify-${seller.id}`}
                        disabled={isStaff}
                        onClick={() => {
                          if (isStaff) {
                            addToast('Staff login cannot edit or modify approved seller records. Admin access required.', 'warning');
                            return;
                          }
                          setModifyingSeller(seller);
                        }}
      ...
      <ViewEditModifyModal
        isOpen={!!modifyingSeller}
        onClose={() => setModifyingSeller(null)}
        entityType="seller"
        record={modifyingSeller}
        onSaveEdit={(updates) => {
          if (modifyingSeller) {
            updateBuyerLead(modifyingSeller.id, updates);
            addToast('Seller updated successfully', 'success');
          }
        }}
        onSendModification={(reason) => {
          if (modifyingSeller) {
            sendBackBuyerForModification(modifyingSeller.id, reason);
            addToast('Seller sent back to Waiting for Modification', 'info');
          }
        }}
      />
    </div>
